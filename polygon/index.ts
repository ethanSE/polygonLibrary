class Polygon {
    ...
    isPchainClosed(chain: Vertex[]): boolean {
        return this.verticesEqual(chain[0], chain[chain.length - 1]);
    }
    getSlope(edge: Edge): number {
    ...
    getLineFromEdge(edge: Edge): Line {
        let xMaxIndex = edge[0].x > edge[1].x ? 0 : 1;
    ...
    getIntersection(line1: Line, line2: Line): Vertex {
    ...
    isPointInRectBounds(edge: Edge, point: Vertex): boolean {
    ...
    doEdgesIntersect(edge1: Edge, edge2: Edge): boolean {
    ...
    isSelfIntersecting(shape: Edge[]): boolean {
